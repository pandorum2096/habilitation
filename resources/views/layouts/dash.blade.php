...
<HEAD>
	...
	<link href="include/assets/vendor/boxicons/css/boxicons.min.css" rel="stylesheet">
	<link href="include/assets/vendor/quill/quill.snow.css" rel="stylesheet">
	<link href="include/assets/vendor/quill/quill.bubble.css" rel="stylesheet">
	<link href="include/assets/vendor/remixicon/remixicon.css" rel="stylesheet">
	<link href="include/assets/vendor/simple-datatables/style.css" rel="stylesheet"> -->
	...
	<script>var HOST_URL = "https://keenthemes.com/metronic/tools/preview";</script>
	<!--begin::Global Config(global config for global JS scripts)-->
	<script>var KTAppSettings = { "breakpoints": { "sm": 576, "md": 768, "lg": 992, "xl":
	1200, "xxl": 1200 }, "colors": { "theme": { "base": { "white": "#ffffff", "primary": "#3699FF", 
	"secondary": "#E5EAEE", "success": "#1BC5BD", "info": "#8950FC", "warning": "#FFA800", "danger": 
	"#F64E60", "light": "#F3F6F9", "dark": "#212121" }, "light": { "white": "#ffffff", "primary": "#E1F0FF",
	"secondary": "#ECF0F3", "success": "#C9F7F5", "info": "#EEE5FF", "warning": "#FFF4DE", "danger": 
	"#FFE2E5", "light": "#F3F6F9", "dark": "#D6D6E0" }, "inverse": { "white": "#ffffff", "primary": 
	"#ffffff", "secondary": "#212121", "success": "#ffffff", "info": "#ffffff", "warning": "#ffffff", 
	"danger": "#ffffff", "light": "#464E5F", "dark": "#ffffff" } }, "gray": { "gray-100": "#F3F6F9", "gray-200":
	"#ECF0F3", "gray-300": "#E5EAEE", "gray-400": "#D6D6E0", "gray-500": "#B5B5C3", "gray-600": "#80808F", 
	"gray-700": "#464E5F", "gray-800": "#1B283F", "gray-900": "#212121" } }, "font-family": "Poppins" };
	</script>
	<!--end::Global Config-->
	<!--  -->
	...
	<script type='text/javascript' src="/include/script/accueil_principal.js"></script>
	...
	<SCRIPT type='text/javascript'>
		...
		var sLibelleTitre = "{CST_DEBUT_TITRE}";
		...
		var sNomApplication = "{NOM_POS_APPLICATION}";

		var CST_ATTENTE_SUPRESSION_DOCUMENT = "{CST_ATTENTE_SUPRESSION_DOCUMENT}";
		...
		var PosTabView = function(id) {
			this.tabViewInner = new YAHOO.widget.TabView(id);
			this.tabViewInner.addListener('activeIndexChange', onChangeTabIndex);
		}

		PosTabView.prototype.selectTab = function(indOnglet) {
			this.tabViewInner.selectTab(indOnglet);
		}

		PosTabView.prototype.addTab = function(id, label, sContent, ind) {
			
			var newtab = new YAHOO.widget.Tab({ label: label, content: sContent});
			newtab.get('contentEl').id = id;
			if (ind == -1)
				this.tabViewInner.addTab(newtab);
			else
				this.tabViewInner.addTab(newtab, ind);
		}

		PosTabView.prototype.removeTab = function(indOnglet) {
			this.tabViewInner.removeTab(this.tabViewInner.get('tabs')[indOnglet]);
		}

		PosTabView.prototype.getActiveIndex = function() {
			return this.tabViewInner.get('activeIndex');
		}

		PosTabView.prototype.getIdTab = function(indOnglet) {
			var tabTab = this.tabViewInner.get('tabs');
			return tabTab[indOnglet].get('contentEl').id;
		}

		PosTabView.prototype.setIdTab = function(indOnglet, id) {
			var tabTab = this.tabViewInner.get('tabs');
			tabTab[indOnglet].get('contentEl').id = id; 
		}

		PosTabView.prototype.setTitle = function(indOnglet, sTitre) {
			this.tabViewInner.getTab(indOnglet).set('label', "&nbsp;" + sTitre);
		}

		PosTabView.prototype.getNbTab = function(indOnglet) {
			return this.tabViewInner.get('tabs').length;
		}
		...
		PosLayout.prototype.openPosDocument = function (szUrlConsulter) {
			...
			if (debutChaine != -1)
			{
				...
				if (indOnglet != -1) {
					...
					this.tabView.addTab( "tab_posdocument_"+iNumDoc, iNumDoc, sContent, -1);
					this.tabView.selectTab(this.tabView.getNbTab() - 1);
				}
			}
		}

		PosLayout.prototype.onConsulterSelected = function (iNumDoc, base_url) {

			var szNumTypDoc;
			var szUrlConsulter;
			for(var i in iNumDoc){

				szUrlConsulter = "";
				szNumTypDoc = [
					"POS_NUM_DOC="+iNumDoc[i].numDoc,
					"POS_TYPEDOC="+iNumDoc[i].typeDoc,
					"isdos="+iNumDoc[i].isDos
				].join("&");
				
				if(arguments.length >= 1)
					szUrlConsulter = base_url+szNumTypDoc;
				else if(Verif_form())
					szUrlConsulter = base_url+getCheckedValue(document.principal.POS_NUMDOC);
				
				/** Modification Andry -  marquage ligne  */

				/*
				if(szUrlConsulter.length > 0)
				{
				

					if($("#doc_"+iNumDoc[i].numDoc).length > 0){
						changerCouleurPolice($("#doc_"+iNumDoc[i].numDoc).get(0));
						$("#doc_"+iNumDoc[i].numDoc).addClass("ligneConsulte");
					}
					
					parent.openPosDocument(szUrlConsulter);

				}
				*/
				/** Modification Andry -  marquage ligne  */

				posLayout.openPosDocument(szUrlConsulter);

			}

		}
		/** Modification Andry -  consultation multiple  */
		...
		PosLayout.prototype.openDocumentCreation = function (szUrlCreer) {
			...
			this.tabView.addTab("tab_posdoc_creation_"+iNbDocumentCreation, "Creation", sContent, -1);
			this.tabView.selectTab(this.tabView.getNbTab() - 1);
			iNbDocumentCreation++;
		}
		...
		PosLayout.prototype.getFramePosDocument = function (iNumDoc) {
			...
			var sName = "ifr_posdocument_"+iNumDoc;
			return frames[sName];   
		}
		...
		PosLayout.prototype.closeOnglet = function(idIFrm)  {
					...
					|| idIFrm.indexOf("ifr_center") != -1)
			{
				...
				if (indOnglet != -1) {
					...
					this.tabView.removeTab(indOnglet);
					if (selectedOngletBeforeClose != -1 && selectedOngletBeforeClose != indOnglet) {
						if (selectedOngletBeforeClose >= this.tabView.getNbTab())
							selectedOngletBeforeClose = this.tabView.getNbTab() - 1;   
		...
		PosLayout.prototype.changeIdFrameCreationToDocument = function(newNumDoc) {
			...
			var iNumOnglet = this.tabView.getActiveIndex();
			if (iNumOnglet != -1) {
				var idTab = this.tabView.getIdTab(iNumOnglet); 
		...
		PosLayout.prototype.changeIdFrame = function (oldId, newId) {
			...
			if (frDoc != null) {
				...
				if (indOnglet != -1)
					this.tabView.setIdTab(indOnglet, "tab_" + newId);
			}
		}
		...
		PosLayout.prototype.findOngletByIdContent = function(sContentOngletId)  {
			...
			var indOnglet = -1;
			var nbTabs = this.tabView.getNbTab();
			for(var i=0;i<nbTabs;i++) {
				if (this.tabView.getIdTab(i) == sContentOngletId) {
					indOnglet = i;
					break;
				}
			}
		...
		PosLayout.prototype.basculerEcran = function(szEcran)  {
			...
			if (szEcran == 'center')
			{
				...
				if (iNumOnglet == -1) {
					this.tabView.addTab("tab_center", "", "", 1);
					document.getElementById('tab_center').appendChild(document.getElementById('ifr_center'));
				}
				try {
		...
		PosLayout.prototype.closeOngletDocByCross = function(sFrmId) {
			...
			selectedOngletBeforeClose = this.tabView.getActiveIndex();
			if(sFrmId.indexOf('ifr_posdoc_creation_') != -1	|| sFrmId == "ifr_center") {
				closeOnglet(sFrmId);
			}
		...
		PosLayout.prototype.setTitle = function()  {
			...
			try {
				var iNumOnglet = this.tabView.getActiveIndex();
				var idFrm = this.tabView.getIdTab(iNumOnglet);
				...
				if (iNumOnglet != -1 && iNumOnglet != 0) {
					...
					if (openDocumentOnConnect == 1 && nameFrm.indexOf("ifr_posdocument") != -1)
						...
						this.tabView.setTitle(iNumOnglet, sTitre)
				}
				
				// anomalie raffraichissement des label sur IE8, la taille de l'onglet ne s'adapte pas
				if (this.tabView.getActiveIndex() && this.tabView.getActiveIndex() >= 0 && 
													this.tabView.getActiveIndex() < this.tabView.getNbTab())
					this.tabView.selectTab(this.tabView.getActiveIndex());
			}
			catch(e) {
			}
		}
		...
		PosLayout.prototype.setTitleDoc = function(nameFrm)  {
			...
			try {
				var nbTab = this.tabView.getNbTab();
				var s = "";
				var idFrm = nameFrm.replace("ifr", "tab");

				var iNumOnglet = -1;
				for(var i=nbTab -1;i>= 1;i--) {
					var idTab = this.tabView.getIdTab(i);
		...
		PosLayout.prototype.closeAllTabs = function()
		{
			try {
				var nbTab = this.tabView.getNbTab();
				for(var i=nbTab - 1;i>= 1;i--) {
					...
					if (idTab.indexOf("tab_posdocument_") != -1 || idTab.indexOf("tab_posdoc_creation_") != -1)
					{
						...
						if (typeof (document.getElementById(idFrm).contentWindow.PosConfirmFermerIndexTestModif) == 'function') {
							//  document.getElementById(idFrm) ramene toujours HTMLIFrameElement  
							if(!document.getElementById(idFrm).contentWindow.PosConfirmFermerIndexTestModif()) {
								return false;
							}
							else {
								document.getElementById(idFrm).contentWindow.FermerIndex();
								window.bLienMenu = 1;
							}
						}
					}
		...
		*/
		...
		function getSelectedRowForOpen(){

			var numsDocChecked = [];
			$("#tablelr_wrapper").find("input.checkbox_select_doc").filter(':checked').each(function(e){
				numsDocChecked.push({
					"numDoc": $(this).parents("tr:first").data("numdoc"),
					"typeDoc": $(this).parents("tr:first").data("typedoc"),
					"isDos": ($(this).parents("tr:first").data("isdos") == "yes") ? 1 : 0 
				});
			});

			return numsDocChecked;
			
		}


		function getSelectedRow(idDrag){

			var numsDocChecked = [];
			var inputCheckboxSelectDoc = parent.document.getElementById('ifr_center').contentWindow.document.getElementsByClassName("checkbox_select_doc");
			
			$(inputCheckboxSelectDoc).filter(':checked').each(function(e){
				numsDocChecked.push($(this).data("id"));
			});
			if(idDrag === null)
				return numsDocChecked;
			else
				document.getElementById(idDrag).posNumDoc = numsDocChecked;

		}

		function openPosDocument(szUrlConsulter, base_url)
		{
			var iNumDoc = getSelectedRowForOpen();
			
			if(iNumDoc.length > 1)
				posLayout.onConsulterSelected(iNumDoc, base_url);
			else
				return posLayout.openPosDocument(szUrlConsulter);
		}
		...
		{
			document.title = "{NOM_APPLICATION}";
		}
		...
		*/ 
		...
		function afficherRechercheTexte()
		{
			...
			posMessageBoxWarning("CST_VALEUR_RECH_VIDE");
			else 
			{
				var szUrl = sUrlAttente + "?URL=../ftx/recherche_ftx.php";
				szUrl += "&POS_CHOIX_LIB=" + escape("ALG;ARC");
				szUrl += "&POS_TRI=0";
...
id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
      <!-- ======= Header ======= -->
      <!--begin::Main-->
		<!--begin::Header Mobile-->
		<div id="kt_header_mobile" class="header-mobile align-items-center header-mobile-fixed ">
			<!--begin::Logo-->
			<a href="index.html">
				<img class="img-dash" alt="Logo" src="core/img/logo-light.png" />
			</a>
			<!--end::Logo-->
			<!--begin::Toolbar-->
					...
					<!--<div class="dropdown">
						<div class="topbar-item" data-toggle="dropdown" data-offset="10px,0px">
								<div class="img-contour  mr-5">
									<div class="img-profil"  style="background-image:url('include/responsives/assets/media/users/300_21.jpg')"></div>
								</div>
						</div>
					</div>-->
					<!--end::Languages-->
		...
		<div class="d-flex flex-column flex-root">
			...
			<div class="d-flex flex-row flex-column-fluid page">
				...
				<div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper">
					...
					<!--end::Header-->
					<!--begin::Content-->
					<div class="back-g content d-flex flex-column flex-column-fluid" id="kt_content" style="padding: 0px; background-image: url('core/img/dash.png'); background-size: contain; background-repeat: round; ">
					...
					<div class="footer bg-white py-4 d-flex flex-lg-column" id="kt_footer">
						...
						<div class="container-fluid d-flex flex-column flex-md-row align-items-center justify-content-between">
							...
							<div class="text-dark order-2 order-md-1">
								...
								<a href="" target="_blank" class="text-dark-75 text-hover-primary">APPNGSYS</a>
							</div>
							<!--end::Copyright-->
							<!--begin::Nav-->
		...
		<!--end::Main-->  
         <!-- begin::User Panel-->
		<div id="kt_quick_user" class="offcanvas offcanvas-right p-10" style="background-image: url('core/img/dashOption.png'); background-size: cover; background-repeat: round;">
			<!--begin::Content-->
			<div class="offcanvas-content pr-5 mr-n5">
				...
				<div class="d-flex align-items-center mt-5 informations">
					<div class="symbol symbol-100 mr-5">
						<div class="symbol-label" style="background-image:url('include/responsives/assets/media/users/300_21.jpg'); border-radius: 50%;"></div>
						<i class="symbol-badge bg-success"></i>
					</div>
					<div class="d-flex flex-column">
						<a href="#" class="text-muted mt-1 btn-txt-panel">{NOM_UTILISATEUR}</a>
						<div class="text-muted mt-1 btn-txt-panel">{DEP_UTILISATEUR}</div>
						<div class="navi mt-2">
							<a href="#" class="navi-item">
								<span class="navi-text text-muted text-hover-primary btn-txt-panel">{PROFIL_UTILISATEUR}</span>
							</a>
						</div>
					</div>
				</div>
				...
				<div class="navi navi-spacer-x-0 p-0">
					...
					<a href="" class="navi-item">
						<div class="navi-link">
							...
							<div class="navi-text">
								<div class="btn-txt-panel2">{CST_PROFIL}</div>
								<div class="btn-txt-panel2-2">{CST_PROFIL_ST}</div>
							</div>
						</div>
					</a>
					<!--end:Item-->
					...
					<a href="custom/apps/user/profile-3.html" class="navi-item">
						<div class="navi-link">
							...
							<div class="navi-text">
								<div class="btn-txt-panel2">{CST_LIBELLE_PWD}</div>
								<div class="btn-txt-panel2-2">{CST_PASSE_ST}</div>
							</div>
						</div>
					</a>
					<a href="custom/apps/userprofile-1/overview.html" class="navi-item">
						<div class="navi-link">
							...
							<div class="navi-text">
								<div class="btn-txt-panel2">{CST_FICHE_POSTE}</div>
								<div class="btn-txt-panel2-2">{CST_FICHE_POSTE_ST}</div>
							</div>
						</div>
					</a>
					<!--end:Item-->
					...
					<a href="custom/apps/userprofile-1/overview.html" class="navi-item">
						<div class="navi-link">
							...
							<div class="navi-text">
								<div class="btn-txt-panel2">{CST_FICHE_CONTRAT}</div>
								<div class="btn-txt-panel2-2">{CST_FICHE_CONTRAT_ST}</div>
							</div>
						</div>
					</a>
					<!--end:Item-->
    ...
    <script type="text/javascript">
        ...
        function updateMenu(tabSzTitreNumDocFils) 
        {
            ...
            $("#treeDiv1").jstree(true).open_node(currentSelectedNode);
        }  
        ...
        */
        ...
        function updateMenuAddQuestionGarderReponse(titreQuestionSansQuote, iNumQuest, iNbReponses, szTypeQuestion) 
        {
            var node = getQuestionSessionNode();
            var typeRecherche = getSzImage(szTypeQuestion);
            var action = (szTypeQuestion == "RECHERCHE_EXCALIBUR") ? "RechFtx" : "Rech";
            $('#treeDiv1').jstree().create_node(node.id, {"text" : titreQuestionSansQuote, 
                "state":{"selected":false,"opened":false}, "type": typeRecherche, "data" : {"action": action, "NumQuest": iNumQuest}}, "last", 
                function(){
                    $("#treeDiv1").jstree(true).open_node(node);
            });
        }
    ...
    <script>
        ...
        setTimeout(fonctionAExecuter(false, "{NOM_UTILISATEUR}", "success", "Bienvenue!"), 6000);

        function fonctionAExecuter(statut,user, type, msg) {
            if (statut == true)
            {
                swal({
                title: msg,
